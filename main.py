...
# V12.3: grafik artık KARARIN VERİLDİĞİ zaman diliminde çizilir.
# Tetik 15m iken grafiğin "TF 1H" demesi, gördüğün şeyle botun baktığı
# şeyin farklı olması demekti.
SIGNAL_CHART_TF      = os.getenv("SIGNAL_CHART_TF", os.getenv("V122_STRUCT_TF", "15m")).strip()
...
# [V12.2'DE KULLANILMIYOR] Tetik artık doğrudan 15m kapanışında (V122_STRUCT_TF).
# Bu bayrak ve v11_ltf_teyit() geriye dönük uyumluluk için duruyor, hiçbir
# karar yolunda okunmuyor. 1H kapanışı hiçbir yerde zorunlu değil.
...
# V12.1 KALİBRASYON — canlı veri (28 Tem) medyan tabanlı eşiğin çöktüğünü gösterdi:
# RE'de işlemlerin %19'u, KORU'da %15'i "balina" işaretlendi. Kripto işlem boyu
# dağılımı ağır kuyrukludur; medyan bu dağılımda taban görevi göremez, 8x medyan
...
# Spoofing
V12_WALL_MULT        = float(os.getenv("V12_WALL_MULT", "6.0"))          # seviye ortalamasının kaç katı = duvar
V12_WALL_MIN_SNAPS   = int(float(os.getenv("V12_WALL_MIN_SNAPS", "3")))  # tek kare parlayan emir duvar sayılmaz
V12_SPOOF_MAX_LIFE   = float(os.getenv("V12_SPOOF_MAX_LIFE_SEC", "30"))  # gerçek spoof kısa yaşar; 90sn çok gevşekti
...
# ============================================================================ #
#  V12.2 — 15m TETİK
#  1H mum kapanışı artık GİRİŞ ŞARTI DEĞİL. Yapı (BOS/CHoCH), pullback ve
#  tetik 15m kapalı mumlardan okunur; 1H yalnızca "aynı yönü söylüyor mu"
#  diye sorulur ve söylüyorsa skora bonus yazılır. Söylemiyorsa CEZA YOK,
#  sadece bonus yok — Hasan'ın mandatı bu.
#  Karşılığında 7 sert şart geldi: 15m yapı + gerçek CVD + balina uyumu +
#  BTC yönü + spread + hacim + spoof riski.
# ============================================================================ #
V122_STRUCT_TF        = os.getenv("V122_STRUCT_TF", "15m").strip()
V122_STRUCT_LIMIT     = int(float(os.getenv("V122_STRUCT_LIMIT", "200")))
V122_1H_CONFIRM       = os.getenv("V122_1H_CONFIRM", "true").lower() == "true"
V122_1H_BONUS         = float(os.getenv("V122_1H_BONUS", "5"))
# RSI kapısı 1H'de kalır: eşikler (LONG max / SHORT min) 1H üzerinde
# kalibre edilmişti; 15m RSI çok daha oynak, aynı eşik başka şey ölçer.
V122_RSI_TF           = os.getenv("V122_RSI_TF", "1h").strip().lower()

# --- 7 sert şart ---
V122_REQUIRE_CVD      = os.getenv("V122_REQUIRE_CVD", "true").lower() == "true"
V122_CVD_MIN_ALIGN    = float(os.getenv("V122_CVD_MIN_ALIGN", "0.10"))
V122_WHALE_MAX_OPPOSE = float(os.getenv("V122_WHALE_MAX_OPPOSE", "0.25"))
V122_BLOCK_ON_SPOOF   = os.getenv("V122_BLOCK_ON_SPOOF", "true").lower() == "true"

# V12.3 — ŞART 6 GERÇEKTEN SERT OLDU.
# APT SHORT (30.07 07:30) hacim x0.36 ile açıldı; hem araştırma hem grafik
# okuyucu "kırılım desteksiz" dedi ama bu sadece 9 puana mal oldu ve sinyal
# 79.2 skorla geçti. Stop yedi. Hasan "hacim filtresi geçerli kalsın" dedi;
# ben onu skor bileşeni olarak bırakmışım — kapı değildi. Artık kapı.
V123_MIN_VOL_RATIO    = float(os.getenv("V123_MIN_VOL_RATIO", "0.8"))
V123_VOL_HARD_GATE    = os.getenv("V123_VOL_HARD_GATE", "true").lower() == "true"

# ============================================================================ #
#  V12.4 — PİVOT ÖNEMİ (yön hatasının kökü)
#  v10_find_swings pivotları BÜYÜKLÜĞÜNE göre ayırmıyordu: %13.7'lik bir
#  çakılmanın dibi ile %0.1'lik bir titreşim aynı ağırlıktaydı. event_side
#  tek değer olduğu için, en SON oluşan mikro pivot işlemin yönünü belirliyordu
#  ve diğer yön hiç değerlendirilmiyordu.
#  APT (30.07 07:30): gerçek yapısal dip 0.5503 (süpürülüp geri alınmış =
#  boğa tablosu) iken bot 0.5690'daki 0.25 ATR'lik kıpırdanmayı "kırılan
#  seviye" sayıp SHORT açtı ve stop yedi.
#  Artık bir pivot, kendinden önceki ters pivottan en az X ATR uzakta
#  değilse YAPI SAYILMAZ — zigzag/prominence mantığı.
# ============================================================================ #
...
V12_WS_TRADE_SYMBOLS  = int(float(os.getenv("V12_WS_TRADE_SYMBOLS", "150")))
...
def v12_on_book(symbol: str, row: Dict[str, Any]) -> None:
    ...
    # --- V12.1 KRİTİK DÜZELTME: books5 PENCERE KENARI ---------------------
    # books5 sadece EN İYİ 5 SEVİYEYİ gösterir. Fiyata daha yakın yeni emirler
    # gelince, 5. seviyedeki emir görüş alanından ÇIKAR — ama iptal edilmiş
    # değildir, hâlâ defterdedir. Eski kod bunu "duvar kayboldu" sayıp spoof
    # yazıyordu. BEAT'te 15 dakikada 34 spoof (2.3/dk) ve iki tarafta da
    # simetrik dağılım tam olarak bunun imzasıydı: manipülasyon değil,
    # sıradan kotasyon trafiği. Artık bir duvar ancak fiyatı HÂLÂ GÖRÜNÜR
    # ARALIKTAYKEN kaybolduysa yargılanıyor; pencereden düşen "bilinmiyor"
    # sayılıp sessizce atılıyor.
    bid_min = min(safe_float(x[0]) for x in bids if len(x) >= 2)
    ask_max = max(safe_float(x[0]) for x in asks if len(x) >= 2)
    ...
    for taraf, seviyeler in (("bid", bids), ("ask", asks)):
        ...
        if not boy:
            continue
        # V12.1: ORTALAMA yerine MEDYAN. Ortalama kullanınca duvarın kendisi
        # tabanı şişiriyordu: [10,900,10,10,10] ortalaması 188, yani 900'lük
        # gerçek bir duvar "ortalamanın 6 katı" eşiğini (1128) geçemiyordu.
        # Medyan (10) tek bir aykırı değerden etkilenmez; duvar artık TİPİK
        # derinliğe göre ölçülüyor — olması gereken de bu.
        sirali = sorted(boy)
        med = sirali[len(sirali) // 2]
        if med <= 0:
            continue
        for lv in seviyeler:
            if len(lv) < 2:
            ...
            if px <= 0 or sz <= med * V12_WALL_MULT:
                continue
    ...
    for key, (px, sz) in simdi.items():
        w = duvarlar.get(key)
        if w is None:
            duvarlar[key] = {"px": px, "max_sz": sz, "ilk": now, "son": now,
                             "degdi": False, "kare": 1}
        else:
            w["son"] = now; w["max_sz"] = max(w["max_sz"], sz); w["kare"] += 1
    # fiyat duvara değdi mi?
    ...
    for key in [k for k in duvarlar if k not in simdi]:
        w = duvarlar.pop(key)
        taraf = key[0]
        # 1) Hâlâ görünür aralıkta mı? Değilse kaderi bilinmiyor → yargılama.
        gorunur = (w["px"] >= bid_min) if taraf == "bid" else (w["px"] <= ask_max)
        if not gorunur:
            continue
        # 2) Tek kare parlayan emir duvar değildir.
        if w.get("kare", 1) < V12_WALL_MIN_SNAPS:
            continue
        omur = w["son"] - w["ilk"]
        if (not w["degdi"]) and omur <= V12_SPOOF_MAX_LIFE:
            b["spoofs"].append({"ts": now, "taraf": taraf, "px": w["px"],
...
def v12_hotlist_books() -> List[str]:
    """books5 (spoofing) — pahalı kanal, dar liste."""
    return v12_hotlist()[:V12_WS_MAX_SYMBOLS]


def v12_hotlist_trades() -> List[str]:
    """trades (CVD + balina) — ucuz kanal, geniş liste.
    CVD V12.2'de SERT ŞART olduğu için taranan evrenin çoğunu kapsamalı."""
    out = list(v12_hotlist_books())
    for s in list(COINS):
        if len(out) >= V12_WS_TRADE_SYMBOLS: break
        s = normalize_symbol(s)
        if s and s not in out: out.append(s)
    return out[:V12_WS_TRADE_SYMBOLS]
    ...
    while True:
        try:
            ...
            async with _ws_lib.connect(V12_WS_URL, ping_interval=None,
                ...
                mevcut_b = set(v12_hotlist_books()) if V12_WS_BOOKS else set()
                if mevcut_t: await _abone(sorted(mevcut_t), "trades")
                if mevcut_b: await _abone(sorted(mevcut_b), "books5")
                logger.info("V12.2 abone: trades=%d books5=%d", len(mevcut_t), len(mevcut_b))
                ...
                async def _hotlist():
                    nonlocal mevcut_t, mevcut_b
                    while True:
                        await asyncio.sleep(V12_WS_HOTLIST_SEC)
                        try:
                            if V12_WS_TRADES:
                                yeni = set(v12_hotlist_trades())
                                ek = yeni - mevcut_t; ck = mevcut_t - yeni
                                if ck: await _abone(sorted(ck), "trades", "unsubscribe")
                                if ek: await _abone(sorted(ek), "trades")
                                for s in ck: _v12_flow.pop(s, None)
                                mevcut_t = yeni
                            if V12_WS_BOOKS:
                                yeni = set(v12_hotlist_books())
                                ek = yeni - mevcut_b; ck = mevcut_b - yeni
                                if ck: await _abone(sorted(ck), "books5", "unsubscribe")
                                if ek: await _abone(sorted(ek), "books5")
                                for s in ck: _v12_book.pop(s, None)
                                mevcut_b = yeni
                        except Exception:
                            return
...
def v12_akis_oku(side: str, symbol: str) -> Dict[str, Any]:
    ...
    if w and w.get("kalibresiz"):
        out["notlar"].append(f"🐳 balina eşiği kalibresiz (işlemlerin %{w['pay']}'i aştı) — yok sayıldı")
        w = None
    if w and w["n"] > 0:
        out["aktif"] = True
    ...
    if s and s["n"] >= V12_SPOOF_MIN_N:
        ...
        if s["egilim"] == "BOĞA İPUCU":
            ...
            ham += 0.0   # V12.1: "KARIŞIK" = iki tarafta da hareket = sıradan
                         # kotasyon trafiği olma ihtimali yüksek. Ceza vermiyoruz.
        if s["egilim"] != "KARIŞIK":
...
def v10_anlamli_pivotlar(sw, atr_deger, min_atr):
    """Gürültü pivotlarını eler (zigzag/prominence).
    Kural 1: arka arkaya aynı türden pivot varsa sadece EN UÇ olanı kalır.
    Kural 2: bir pivot, kendinden önceki KORUNAN ters pivottan en az
             min_atr × ATR uzakta değilse yapı sayılmaz."""
    if not V124_SWING_FILTER or atr_deger <= 0 or min_atr <= 0 or not sw:
        return sw
    esik = atr_deger * min_atr
    out = []
    for x in sw:
        if not out:
        ...
        son = out[-1]
        if x.kind == son.kind:
            daha_uc = (x.price > son.price) if x.kind == "H" else (x.price < son.price)
            if daha_uc: out[-1] = x
            continue
        if abs(x.price - son.price) < esik:
            continue                      # yeterince belirgin değil → gürültü
        out.append(x)
    return out
...
def v10_market_structure(k):
    ...
    # V12.4: gürültü pivotlarını ele. Bu satır olmadan en son oluşan mikro
    # kıpırdanma event_side'ı belirliyor ve işlemin yönünü tek başına çeviriyor.
    try:
        _a = atr(k, V10_ATR_PERIOD)[-1]
        sw = v10_anlamli_pivotlar(sw, _a, V124_SWING_MIN_ATR)
    except Exception:
        pass
...
async def v11_deep_research(sig: Dict[str, Any], k1h: List[List[Any]]) -> Dict[str, Any]:
    ...
    try:
        ...
        t1h = str(sig.get("trend_struct", sig.get("trend_1h", "RANGE"))).upper()
...
def v122_akis_kapisi(side: str, symbol: str,
    ...
    Hepsi WS bellek içi okumadır; API çağrısı yok, bedava.
    Şart 4 (BTC), 5 (spread), 6 (hacim) derin araştırma kapısında zaten sert."""
    bilgi: Dict[str, Any] = {}
    up = (side == "LONG")

    # --- ŞART 2: GERÇEK CVD (sert) ---
    c = v12_cvd(symbol, 900) if V12_WS_ENABLED else None
    bilgi["cvd"] = c
    if V122_REQUIRE_CVD:
        if not v12_health()["ok"]:
    ...
    bilgi["whale"] = w
    if w and w.get("n", 0) > 0:
        uyum = w["baski"] if up else -w["baski"]
        if uyum < -V122_WHALE_MAX_OPPOSE:
            return False, (f"balina baskısı TERS ({w['baski']:+.2f}, "
                           f"{w['n']} baskı)"), bilgi
    # veri yoksa "ters değil" kabul edilir — mandat böyle

    # --- ŞART 7: SPOOF riski yüksekse sinyal yok ---
    s = v12_spoof(symbol) if V12_WS_ENABLED else None
    bilgi["spoof"] = s
    if V122_BLOCK_ON_SPOOF and s and s.get("n", 0) >= V12_SPOOF_MIN_N:
        eg = s.get("egilim")
        if eg == "KARIŞIK":
            return False, f"spoof riski: iki tarafta oyun ({s['bid']}alış/{s['ask']}satış)", bilgi
        if up and eg == "AYI İPUCU":
            return False, f"spoof LONG'a karşı: {s['not']}", bilgi
        if (not up) and eg == "BOĞA İPUCU":
            return False, f"spoof SHORT'a karşı: {s['not']}", bilgi
    return True, "", bilgi
...
async def analyze_v10_symbol(symbol: str) -> Optional[Dict[str, Any]]:
    ...
    # okunuyor. 1H kapanışı beklenmiyor; hiçbir yerde zorunlu değil.
    ...
    # 1H artık TEYİT — şart değil. Aynı yönü söylüyorsa bonus, demiyorsa ceza yok.
    k1h = await get_klines(symbol, MA_KLINE_INTERVAL, V10_KLINE_LIMIT)
    h1_teyit = None; h1_bonus = 0.0; ltf_note = ""; trend_1h_ger = "-"
    if len(k1h) >= 40:
        _ms1 = v10_market_structure(_s_closed(k1h))
        trend_1h_ger = _ms1.get("trend", "-")
    if V122_1H_CONFIRM and len(k1h) >= 40:
        ms1 = _ms1
    ...
    # başka bir şey ölçer ve kapı istemeden gevşer/sıkışır.
    if V122_RSI_TF == "1h" and len(k1h) >= 20:
        r = round(rsi(closes(_s_closed(k1h)))[-1], 1)
        parts["rsi_tf"] = "1H"
    if h1_bonus:
    ...
    # ATR 1H'den alınıyor: TP/SL hesabı DEĞİŞMESİN mandatı. (Varsayılan
    # V11_TARGET_MODE=yuzde olduğu için stop zaten sabit %2 — ATR sadece
    # env ile ATR moduna dönülürse devreye girer; o durumda da stop
    # genişliği eskisiyle aynı kalsın diye 1H serisi kullanılıyor.)
    a_kaynak = k1h if len(k1h) >= V10_ATR_PERIOD + 2 else k
    a = atr(a_kaynak, V10_ATR_PERIOD)[-1]; tgt = v10_targets(side, entry, a, fib)
...
# V12.3: araştırma notları eskiden HEPSİ "✓" ile basılıyordu. APT sinyalinde
# "✓ hacim x0.36 ZAYIF — kırılım desteksiz" yazıyordu; onay işareti bir
# UYARIYI teyit gibi gösteriyordu. Artık bulgunun tonuna göre işaret veriliyor.
_V123_UYARI = ("zayıf", "desteksiz", "ters", "karışık", "kısmi", "uçta",
               "geç)", "yetersiz", "riski", "hatası")
_V123_NOTR  = ("nötr", "normal", "yok", "dengeli")


def _v123_isaret(n: str) -> str:
    t = _v11_lower(n)
    if any(k in t for k in _V123_UYARI): return "⚠"
    if any(k in t for k in _V123_NOTR):  return "•"
    return "✓"
...
async def cmd_ws(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    lines += [
        ...
        f"Abone kanal: {h['abone']} | trades hedef: {V12_WS_TRADE_SYMBOLS} | books5 hedef: {V12_WS_MAX_SYMBOLS}",
    ...
    for sym in list(_v12_flow.keys())[:6]:
        ...
        if w and w.get("kalibresiz"):
            par.append(f"balina KALİBRESİZ (%{w['pay']}) — yok sayılıyor")
        elif w and w["n"]:
            par.append(f"balina {w['n']} (%{w.get('pay',0)}) baskı {w['baski']:+.2f}")
        if s and s["n"]:
            par.append(f"spoof {s['bid']}alış/{s['ask']}satış → {s['egilim']}")
...
async def cmd_v10(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    lines = [
        ...
        f"⚡ TETİK: {V122_STRUCT_TF} kapanış (1H ZORUNLU DEĞİL) | 1H teyidi bonus +{V122_1H_BONUS:g} | "
        f"teyit gelen={int(stats.get('v122_1h_teyit',0))}",

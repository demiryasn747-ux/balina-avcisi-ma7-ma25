...
VERSION_NAME = "Balina Avcısı V11 HİBRİT-MTF (1H tetik + 4H trend + ATR/RR + RiskGuard + Öğrenen Skor)"
...
async def analyze_v10_symbol(symbol: str) -> Optional[Dict[str, Any]]:
    ...
    v11_keys, v11_snap = v11_extract_features(
        side, gate["ms"]["event"], gate["ms"]["trend"], gate["trend4"],
    ...
    return {"symbol":symbol,"direction":side,"entry":entry,"strategy":"V10_SMC",
            ...
            "v11_keys":v11_keys,"v11_snap":v11_snap,
...
V11_ADJ_FAMILIES = set(x.strip() for x in os.getenv(
    "V11_ADJ_FAMILIES",
    "skor,yon,olay,1H+olay,btc1h,btc1h+yon,btc4h,fomo,oi,fund").split(",") if x.strip())
...
def v11_extract_features(side, event, trend_1h, trend_4h, btc_1h, btc_4h,
                         fomo, oi, funding, r, base_score, parts):
    ...
    ] + [f"onay:{n}={v}" for n, v in fl.items()]
...
def v11_score_adjust(keys):
    """Aktif öğrenme ayarlarını sinyale uygular → (toplam ayar, kova bazlı döküm)."""
    if not V11_LEARN_ENABLED:
        return 0.0, []
    adj = _v11_mem().get("adjust", {})
    det = [(kk, adj[kk]) for kk in keys if kk in adj]
...
def v11_recalc():
    ...
    for kk, s in stats_k.items():
        fam = kk.split(":", 1)[0]
        if fam not in V11_ADJ_FAMILIES or s["n"] < V11_MIN_BUCKET_N:
            continue
        wr = s["win"] / s["n"]
        d = clamp(V11_ADJ_GAIN * (wr - base_wr), -V11_ADJ_PER_BUCKET, V11_ADJ_PER_BUCKET)
        if abs(d) >= 0.5:
            adjust[kk] = round(d, 1)
    m["adjust"] = adjust
    m["adjust_meta"] = {

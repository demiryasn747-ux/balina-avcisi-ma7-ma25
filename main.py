...
# --- Funding-Ekstrem motoru (kalabalığın tersine dönüş) ---
FUNDING_POS_THRESH = float(os.getenv("FUNDING_POS_THRESH", "0.0005"))   # funding ≥ bu → long kalabalığı → SHORT
FUNDING_NEG_THRESH = float(os.getenv("FUNDING_NEG_THRESH", "0.0005"))   # funding ≤ -bu → short kalabalığı → LONG
FUNDING_REQUIRE_CONFIRM = os.getenv("FUNDING_REQUIRE_CONFIRM", "true").lower() == "true"  # fiyat dönüş teyidi şart
FUNDING_CONFIRM_PCT = float(os.getenv("FUNDING_CONFIRM_PCT", "0.3"))    # teyit için min fiyat dönüşü %
FUNDING_PRICE_BARS = int(float(os.getenv("FUNDING_PRICE_BARS", "3")))   # fiyat değişim penceresi (1H bar)
# --- Paper P&L defteri (canlı kâğıt-ticaret; backtestlenemeyen motorları ölçmek için) ---
PAPER_MAX_TRADES = int(float(os.getenv("PAPER_MAX_TRADES", "1000")))   # defterde tutulacak max trade
PAPER_MAX_HOLD_HOURS = float(os.getenv("PAPER_MAX_HOLD_HOURS", "96"))  # bu süre TP/STOP gelmezse son fiyattan kapat (timeout)
PAPER_RESET_ON_DEPLOY = os.getenv("PAPER_RESET_ON_DEPLOY", "false").lower() == "true"
...
def detect_funding_extreme(funding_rate: float, price_change_pct: float,
                           pos_thresh: float, neg_thresh: float,
                           require_confirm: bool, confirm_pct: float) -> Optional[str]:
    """
    Funding ekstremi → kalabalığın tersine:
    - funding ≥ pos_thresh: long kalabalığı sıkışmış → SHORT (teyit: fiyat dönmeye başlamış olmalı).
    - funding ≤ -neg_thresh: short kalabalığı → LONG.
    """
    if funding_rate >= pos_thresh:
        if require_confirm and price_change_pct > -confirm_pct:
            return None  # fiyat henüz aşağı dönmedi → düşen bıçağı tutma
        return "SHORT"
    if funding_rate <= -neg_thresh:
        if require_confirm and price_change_pct < confirm_pct:
            return None  # fiyat henüz yukarı dönmedi
        return "LONG"
    return None


def build_funding_signal(symbol, klines_1h, funding_rate,
                         balance_usdt=1000.0, risk_pct=1.5, leverage=1.0, atr_mult=1.6) -> Optional[Dict[str, Any]]:
    """Funding-ekstrem sinyali: kalabalığın tersine + fiyat dönüş teyidi + stop/TP + liq guard + sizing."""
    closed = _s_closed(klines_1h)
    cl = _s_closes(closed)
    if len(cl) < max(FUNDING_PRICE_BARS + 1, 16):
        return None
    entry = cl[-1]
    if entry <= 0:
        return None
    past = cl[-1 - FUNDING_PRICE_BARS]
    price_change_pct = (entry - past) / past * 100.0 if past > 0 else 0.0
    side = detect_funding_extreme(funding_rate, price_change_pct,
                                  FUNDING_POS_THRESH, FUNDING_NEG_THRESH,
                                  FUNDING_REQUIRE_CONFIRM, FUNDING_CONFIRM_PCT)
    if side is None:
        return None
    ...
    return {
        ...
        "funding_rate": funding_rate, "price_change_pct": round(price_change_pct, 2),
        "mtf_note": f"Funding ekstrem {side}: funding {funding_rate*100:.4f}%/8h (kalabalığın tersine) + fiyat {price_change_pct:+.2f}%",
        "strategy": "FUNDING",
    }


def _bt_funding_at(funding_sorted: List[Any], ts: int) -> float:
    """ts anında geçerli funding oranı = ts'ten önceki son settlement. funding_sorted ascending (ft, rate)."""
    rate = 0.0
    for ft, r in funding_sorted:
        if ft <= ts:
            rate = r
        else:
            break
...
async def get_btc_trend_bias() -> Dict[str, Any]:
    ...
    if cached is not None and (now - _BTC_BIAS_CACHE.get("ts", 0)) < BTC_BIAS_CACHE_SEC:
        return cached
    bias = {"trend": "FLAT", "strong": False, "gap_pct": 0.0, "atr_pct": 0.0, "dir_1h": "FLAT",
            "regime": "RANGE", "ok": False}
    try:
        ...
        if len(k4h) >= 20:
            ...
            bias = {"trend": trend, "strong": strong, "gap_pct": round(gap_pct, 2),
                    "atr_pct": round(atr_pct, 2), "dir_1h": dir_1h,
                    "regime": ("TREND" if (strong and trend in ("UP", "DOWN")) else "RANGE"),
                    "ok": True}
    except Exception as e:
...
def mark_ma_sent(res: Dict[str, Any]) -> None:
    ...
    memory.setdefault("ma_follows", {})[key] = {
        ...
        "highest_price": res["entry"] if res["direction"] == "LONG" else 0.0, # Long için en yüksek fiyat
        "lowest_price": res["entry"] if res["direction"] == "SHORT" else 0.0, # Short için en düşük fiyat
...
async def analyze_hybrid_symbol(symbol: str) -> Optional[Dict[str, Any]]:
    ...
    market_regime = detect_market_regime(k1h, k4h) # k4h burada zaten çekilmiş olmalı
    ...
    # MTF onayları için gerekli klines
...
async def run_hybrid_backtest(symbols: Optional[List[str]] = None, days: int = 30) -> str:
    ...
    for symbol in symbols[:BT_MAX_COINS]:
        try:
            ...
            fhist = await _bt_funding_history(symbol, int(safe_float(k1h[0][0]))) if (BT_REAL_FUNDING or SIGNAL_ENGINE == "funding") else []
...
def _paper_r(entry: float, stop: float, exit_price: float, direction: str) -> float:
    """Kapanan paper trade'in NET R'si (slippage+fee dahil; backtest maliyet modeliyle tutarlı)."""
    risk_unit = abs(entry - stop)
    if risk_unit <= 0:
        return 0.0
    gross = (exit_price - entry) if direction == "LONG" else (entry - exit_price)
    r_gross = gross / risk_unit
    cost_r = ((BT_SLIPPAGE_PCT + BT_FEE_PCT * 2) * entry) / risk_unit
    return round(r_gross - cost_r, 3)


def paper_record(res: Dict[str, Any]) -> None:
    """Gönderilen her sinyali paper defterine OPEN olarak yaz (tüm motorlar için)."""
    if not res:
        return
    trades = memory.setdefault("paper_trades", [])
    pid = f"{res.get('symbol')}:{res.get('direction')}:{res.get('candle_ts','')}"
    if any(t.get("id") == pid for t in trades):
        return
    trades.append({
        "id": pid, "ts": time.time(), "entry_ts_ms": int(safe_float(res.get("candle_ts", 0))),
        "symbol": res.get("symbol", ""), "dir": res.get("direction", ""),
        "engine": res.get("strategy", SIGNAL_ENGINE.upper()),
        "entry": safe_float(res.get("entry")), "stop": safe_float(res.get("stop")),
        "tp1": safe_float(res.get("tp1")), "tp2": safe_float(res.get("tp2")), "tp3": safe_float(res.get("tp3")),
        "status": "OPEN", "exit": 0.0, "r": 0.0, "result": "",
    })
    if len(trades) > PAPER_MAX_TRADES:
        del trades[:len(trades) - PAPER_MAX_TRADES]


async def paper_check_open() -> None:
    """Açık paper trade'leri canlı mum verisine karşı TP/STOP simüle eder; çözülenleri R ile kapatır."""
    trades = memory.get("paper_trades", [])
    open_tr = [t for t in trades if t.get("status") == "OPEN"]
    for t in open_tr:
        try:
            k = await get_klines(t["symbol"], MA_KLINE_INTERVAL, 120)
        except Exception:
            continue
        if not k:
            continue
        ets = int(t.get("entry_ts_ms", 0))
        future = [c for c in k if int(safe_float(c[0])) > ets]
        if len(future) < 1:
            continue
        sim = bt_simulate_tp_stop(t["entry"], t["stop"], t["tp1"], t["tp2"], t["tp3"], future, t["dir"])
        age_h = (time.time() - safe_float(t.get("ts", time.time()))) / 3600.0
        if sim["result"] == "OPEN":
            if age_h >= PAPER_MAX_HOLD_HOURS:
                exitp = safe_float(future[-1][4])
                async with memory_lock:
                    t["status"] = "CLOSED"; t["result"] = "TIMEOUT"
                    t["exit"] = exitp; t["r"] = _paper_r(t["entry"], t["stop"], exitp, t["dir"])
            continue
        exitp = safe_float(sim.get("exit_price", t["entry"]))
        async with memory_lock:
            t["status"] = "CLOSED"; t["result"] = sim["result"]
            t["exit"] = exitp; t["r"] = _paper_r(t["entry"], t["stop"], exitp, t["dir"])
        logger.info("PAPER kapandı %s %s %s R=%.2f", t["symbol"], t["dir"], sim["result"], t["r"])
...
def paper_summary() -> str:
    ...
    if not closed:
        return (f"📒 PAPER DEFTERİ — BUILD {BOT_BUILD} | Motor: {SIGNAL_ENGINE.upper()}\n"
                f"Kapanan trade yok. Açık pozisyon: {open_n}\n"
                f"(Sinyaller gelip TP/STOP'a ulaştıkça dolacak — sabırla.)")
    rs = [safe_float(t["r"]) for t in closed]
    n = len(rs)
    wins = [r for r in rs if r > 0]
    losses = [r for r in rs if r <= 0]
    wr = len(wins) / n * 100.0
    ...
    eq = 1000.0
    for r in rs:
        eq *= (1 + r * HYBRID_RISK_PCT / 100.0)
    ret = (eq / 1000.0 - 1) * 100.0

    def grp(key):
        g: Dict[str, List[float]] = {}
        for t in closed:
            g.setdefault(str(t.get(key, "?")), []).append(safe_float(t["r"]))
        return g

    dist: Dict[str, int] = {}
    for t in closed:
        dist[str(t.get("result", "?"))] = dist.get(str(t.get("result", "?")), 0) + 1
    lines = [f"📒 PAPER DEFTERİ — BUILD {BOT_BUILD} | {n} kapanan (açık: {open_n})",
             f"Kazanma %{wr:.1f} | beklenti {exp:+.2f}R | PF {pf:.2f}",
             f"Ort. kazanç {avg_w:+.2f}R | ort. kayıp {avg_l:+.2f}R",
             f"Getiri (compound %{HYBRID_RISK_PCT}/trade): {ret:+.1f}% → {eq:.0f} USDT",
             "━━ YÖN ━━"]
    for d, rl in grp("dir").items():
        lines.append(f"  {d}: {len(rl)} | %{sum(1 for x in rl if x>0)/len(rl)*100:.0f} | {sum(rl)/len(rl):+.2f}R")
    lines.append("━━ MOTOR ━━")
    for en, rl in grp("engine").items():
        lines.append(f"  {en}: {len(rl)} | {sum(rl)/len(rl):+.2f}R")
    lines.append("━━ ÇIKIŞ ━━ " + " ".join(f"{k}={v}" for k, v in dist.items()))
    if n < 30:
        lines.append("⚠️ <30 trade — sonuç henüz güvenilir değil, biriksin.")
    return "\n".join(lines)
...
async def cmd_version(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Hangi sürüm canlı + kritik ayarlar (deploy doğrulaması)."""
    txt = (
        f"🔖 BUILD: {BOT_BUILD}\n"
        f"Motor (SIGNAL_ENGINE): {SIGNAL_ENGINE.upper()}\n"
        f"Sabit %TP: {'açık' if HYBRID_FIXED_TARGETS else 'kapalı (RR)'}"
        + (f" — stop %{HYBRID_FIXED_STOP_PCT:g} / TP %{HYBRID_FIXED_TP1_PCT:g}/{HYBRID_FIXED_TP2_PCT:g}/{HYBRID_FIXED_TP3_PCT:g}"
...
async def cmd_paper(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Paper defteri raporu: gönderilen sinyallerin kümülatif R sonuçları."""
    await update.message.reply_text(paper_summary())
...
async def cmd_whaletest(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    lines = [f"🐋 BALİNA VERİSİ ERİŞİM TESTİ — BUILD {BOT_BUILD} (OKX, Railway'den)\n"]
...
#  V11 ÖĞRENEN SKOR: her sinyal açılışında TÜM özellikler kaydedilir; kapanışta
#  sonuç STOP/TP1/TP2/TP3 olarak yazılır; her V11_RECALC_EVERY işlemde kova
#  başarı oranları yeniden hesaplanır ve puanlar otomatik ayarlanır
#  (memory["v11"], rapor: /v11 komutu).
#  V11.1 KATMANLARI: ⚡ hızlı pencere (son 30 işlemde kötüleşen kovanın cezası
#  anında artar) + coin hafızası (coin+olay: hangi coin BOS/CHoCH ile çalışır)
#  + piyasa rejimi (TREND/RANGE ayrı öğrenme) + dinamik bileşen ağırlıkları
#  (OI/BTC/funding... etkisi geçmiş ayırt ediciliğe göre 0.6x–1.4x ölçeklenir).
...
def v10_quality_score(side, k, ms, ext):
    ...
    # V11 dinamik ağırlıklar: bileşen çarpanları öğrenilmiş performansa göre;
    # toplam teorik max yeniden 100'e normalize edilir (eşik anlamı bozulmaz).
    wgt = _v11_mem().get("weights") or {}
    if wgt:
        mx = {"structure": 18.0, "volume": 2.0, "rsi": 7.0, "oi": 9.0, "funding": 7.0,
              "btc": 14.0, "orderbook": 7.0, "order_block": 11.0, "fvg": 8.0,
              "volume_profile": 5.0, "cvd": 5.0, "sweep": 7.0}
        tot = sum(mx[kk] * safe_float(wgt.get(kk, 1.0), 1.0) for kk in mx)
        norm = 100.0 / tot if tot > 0 else 1.0
        p = {kk: vv * safe_float(wgt.get(kk, 1.0), 1.0) * norm for kk, vv in p.items()}
...
async def analyze_v10_symbol(symbol: str) -> Optional[Dict[str, Any]]:
    ...
    if (side == "LONG" and r > V10_RSI_LONG_MAX) or (side == "SHORT" and r < V10_RSI_SHORT_MIN):
        return None
    # ---- V11: tüm özellikleri çıkar + öğrenilmiş puan ayarını uygula ----
    regime = btc_bias.get("regime", "RANGE")
    v11_keys, v11_snap = v11_extract_features(
...
def build_v10_message(sig):
    ...
    return (f"🎯 {VERSION_NAME}\n🆕 V10 SMC | {sig['direction']} | {sig['symbol']}\n"
            ...
            f"Skor: {sig['score']}/100{_v11_score_note(sig)}  RSI:{sig['rsi']}\nConfluence: {conf}\n"
...
# ============================================================================ #
#  V11 ÖĞRENEN SKOR MOTORU
#  1) Her sinyal açıldığında TÜM özellikler kaydedilir (v11_keys + v11_snap).
#  2) Pozisyon kapanınca sonuç STOP / TP1 / TP2 / TP3 olarak yazılır.
#  3) Her V11_RECALC_EVERY kapanışta istatistikler yeniden hesaplanır.
#  4) Kova başarı oranı genel ortalamayla kıyaslanır → puan otomatik ayarlanır:
#     ayar = V11_ADJ_GAIN × (kova_başarı − genel_başarı), kova başına
#     ±V11_ADJ_PER_BUCKET puan, toplamda ±V11_ADJ_TOTAL puan sınırı.
#     Giriş eşiği (V10_MIN_QUALITY) nihai (ayarlanmış) skora uygulanır.
#     Örn: oi:>0.5 başarı %71, genel %55 → +3.2p | btc1h+yon:DOWN+LONG %36 → -3.8p
#  Öğrenme, geri-besleme kirliliği olmasın diye HAM (baz) skor üzerinden
#  kovalanır; ayar sadece nihai skora ve eşiğe yansır.
# ============================================================================ #
V11_LEARN_ENABLED   = os.getenv("V11_LEARN_ENABLED", "true").lower() == "true"
V11_RECALC_EVERY    = int(float(os.getenv("V11_RECALC_EVERY", "100")))   # 100–200 arası önerilir
V11_MIN_BUCKET_N    = int(float(os.getenv("V11_MIN_BUCKET_N", "15")))    # kova ayarı için min örnek
V11_WINDOW          = int(float(os.getenv("V11_WINDOW", "500")))         # son N işlem üzerinden öğren
V11_TRADES_KEEP     = int(float(os.getenv("V11_TRADES_KEEP", "1500")))   # memory'de tutulan max kayıt
V11_ADJ_GAIN        = float(os.getenv("V11_ADJ_GAIN", "20"))             # puan = gain × (wr − baz_wr)
V11_ADJ_PER_BUCKET  = float(os.getenv("V11_ADJ_PER_BUCKET", "5"))
V11_ADJ_TOTAL       = float(os.getenv("V11_ADJ_TOTAL", "15"))
...
V11_FAST_WINDOW     = int(float(os.getenv("V11_FAST_WINDOW", "30")))
V11_FAST_MIN_N      = int(float(os.getenv("V11_FAST_MIN_N", "8")))
V11_FAST_GAIN       = float(os.getenv("V11_FAST_GAIN", "15"))
V11_FAST_PER_BUCKET = float(os.getenv("V11_FAST_PER_BUCKET", "4"))
# --- Piyasa rejimi: bu aileler TREND ve RANGE için AYRI AYRI öğrenilir ---
#     Rejim-önekli kova (örn TREND|oi:>0.5) kendi rejiminin baz oranına göre ölçülür.
V11_REGIME_FAMILIES = set(x.strip() for x in os.getenv(
    "V11_REGIME_FAMILIES",
    "fomo,oi,btc1h,btc1h+yon,olay,1H+olay").split(",") if x.strip())
# --- Dinamik filtre ağırlıkları: bileşenin kova-başarı YAYILIMI ayırt ediciliğidir ---
#     Yayılım büyük → bileşen öngörücü → skor ağırlığı artar (0.6x–1.4x), 100 ölçeği korunur.
V11_W_ENABLED = os.getenv("V11_W_ENABLED", "true").lower() == "true"
V11_W_GAIN    = float(os.getenv("V11_W_GAIN", "2.0"))
V11_W_NEUTRAL = float(os.getenv("V11_W_NEUTRAL", "0.10"))   # %10 yayılım = nötr (1.0x)
V11_W_MIN     = float(os.getenv("V11_W_MIN", "0.6"))
V11_W_MAX     = float(os.getenv("V11_W_MAX", "1.4"))


def _v11_mem():
    m = memory.setdefault("v11", {})
    m.setdefault("trades", [])        # her kapanan işlemin TAM kaydı
    m.setdefault("buckets", {})       # tüm-zaman kova sayaçları (rapor)
    m.setdefault("adjust", {})        # aktif puan ayarları {kova: ±puan}
    m.setdefault("adjust_meta", {})   # son recalc istatistikleri
    m.setdefault("fast_adjust", {})   # son 30 işlem penceresinden hızlı ±puan
    m.setdefault("fast_meta", {})
    m.setdefault("weights", {})       # dinamik skor bileşen çarpanları
    m.setdefault("n_closed", 0)
    m.setdefault("last_recalc_n", 0)
    return m
...
def v11_fomo_band(mv):
    a = abs(safe_float(mv))
    if a <= 0.6: return "0-0.6"
    if a <= 1.2: return "0.6-1.2"
    return ">1.2"
...
def v11_oi_band(oi):
    ...
    if oi < -1: return "<-1"
    if oi < 0: return "-1..0"
    if oi <= 0.5: return "0..0.5"
    return ">0.5"


def v11_fund_band(fr):
    fr = safe_float(fr)
    if fr < -0.0001: return "NEG"
    if fr > 0.0001: return "POZ"
    return "NOTR"
...
def v11_rsi_band(r):
    r = safe_float(r)
    if r < 35: return "<35"
    if r < 50: return "35-50"
    if r < 65: return "50-65"
    return "65+"


def _v11_fam(key):
    """Kova ailesini döndürür; rejim öneki varsa soyar: 'TREND|oi:>0.5' → 'oi'."""
    k = key.split("|", 1)[1] if "|" in key else key
    return k.split(":", 1)[0]


def v11_extract_features(symbol, side, event, trend_1h, trend_4h, btc_1h, btc_4h,
                         fomo, oi, funding, r, base_score, parts, regime="RANGE"):
    """Sinyal açılış anındaki BÜTÜN özellikleri kova anahtarlarına + tam snapshot'a çevirir.
    İçerir: coin hafızası (coin, coin+olay), piyasa rejimi (rejim kovası +
    rejim-önekli kovalar → aynı kural TREND ve RANGE'de ayrı öğrenilir)."""
    base_sym = _base_of(symbol)
    fl = {n: (1 if safe_float((parts or {}).get(k)) > 0 else 0)
          for n, k in (("OB", "order_block"), ("FVG", "fvg"), ("SWEEP", "sweep"),
                       ("CVD", "cvd"), ("VP", "volume_profile"), ("KITAP", "orderbook"))}
    core = [
        f"skor:{v11_skor_band(base_score)}",
        f"yon:{side}",
        f"olay:{event}",
        f"1H+olay:{trend_1h}+{event}",          # örn: DOWN+CHoCH, RANGE+BOS
        f"btc1h:{btc_1h}",
        f"btc1h+yon:{btc_1h}+{side}",           # örn: DOWN+LONG
        f"btc4h:{btc_4h}",
        f"fomo:{v11_fomo_band(fomo)}",
        f"oi:{v11_oi_band(oi)}",
        f"fund:{v11_fund_band(funding)}",
        f"rsi:{v11_rsi_band(r)}",
        f"rejim:{regime}",                       # piyasa rejimi kovası
        f"coin:{base_sym}",                      # coin bazlı hafıza
        f"coin+olay:{base_sym}+{event}",         # örn: EGLD+BOS vs EGLD+CHoCH
    ]
    reg_keys = [f"{regime}|{kk}" for kk in core if _v11_fam(kk) in V11_REGIME_FAMILIES]
    keys = core + reg_keys + [f"onay:{n}={v}" for n, v in fl.items()]
    snap = {"score_base": safe_float(base_score), "side": side, "event": event,
            "symbol": base_sym, "regime": regime,
            "trend_1h": trend_1h, "trend_4h": trend_4h,
            "btc_1h": btc_1h, "btc_4h": btc_4h,
            "fomo": round(safe_float(fomo), 3), "oi": round(safe_float(oi), 3),
            "funding": safe_float(funding), "rsi": round(safe_float(r), 1),
            "flags": fl}
    return keys, snap


def v11_score_adjust(keys):
    """Yavaş (pencere) + hızlı (son 30) öğrenme ayarlarını sinyale uygular."""
    if not V11_LEARN_ENABLED:
        return 0.0, []
    m = _v11_mem()
    slow = m.get("adjust", {}) or {}
    fast = m.get("fast_adjust", {}) or {}
    det = []
    for kk in keys:
        d = safe_float(slow.get(kk, 0)) + safe_float(fast.get(kk, 0))
        if d:
            det.append((kk, round(d, 1)))
    tot = clamp(sum(d for _, d in det), -V11_ADJ_TOTAL, V11_ADJ_TOTAL)
    return round(tot, 1), det


def _v11_score_note(sig):
    a = safe_float(sig.get("v11_adjust"))
    if not a:
        return ""
    return f" (baz {safe_float(sig.get('score_base', sig.get('score'))):g}, öğrenme {a:+.1f})"


def v11_result_level(pos, outcome):
    """Kapanışı 4 sınıfa indirger: STOP / TP1 / TP2 / TP3.
    BE kapanışları ulaşılan en yüksek TP seviyesi olarak sayılır."""
    if outcome == "TP3": return "TP3"
    if outcome == "STOP": return "STOP"
    if pos.get("hit2"): return "TP2"
    if pos.get("hit1"): return "TP1"
    return "STOP"


def v11_record_trade(pos, R, outcome):
    """Pozisyon kapanınca sonucu + açılıştaki tüm özellikleri kaydeder."""
    if not pos.get("v11_keys") and not pos.get("v11_snap"):
        return  # V11 öncesi açılmış eski pozisyon — özelliksiz kayıt öğrenmeyi bozar
    m = _v11_mem()
    keys = pos.get("v11_keys") or []
    res = v11_result_level(pos, outcome)
    win = res != "STOP"
    m["trades"].append({
        "ts": time.time(), "symbol": pos.get("symbol"), "side": pos.get("side"),
        "R": round(safe_float(R), 3), "outcome": outcome, "result": res,
        "score": pos.get("score"), "score_base": pos.get("score_base", pos.get("score")),
        "keys": keys, "snap": pos.get("v11_snap", {}),
    })
    if len(m["trades"]) > V11_TRADES_KEEP:
        m["trades"] = m["trades"][-V11_TRADES_KEEP:]
    for kk in keys:
        b = m["buckets"].setdefault(kk, {"n": 0, "win": 0, "R": 0.0})
        b["n"] += 1
        b["R"] = round(b["R"] + safe_float(R), 3)
        if win:
            b["win"] += 1
    m["n_closed"] = int(m.get("n_closed", 0)) + 1
    try:
        v11_fast_recalc()   # son 30 işlem penceresi her kapanışta tazelenir
    except Exception as e:
        logger.exception("v11_fast_recalc hata: %s", e)
    logger.info("V11 KAYIT %s %s sonuç=%s R=%.2f (toplam kapanan=%d)",
                pos.get("side"), pos.get("symbol"), res, R, m["n_closed"])


def v11_fast_recalc():
    """HIZLI ÖĞRENME: son V11_FAST_WINDOW (30) işlemi tarar; kötüleşen kovanın
    cezasını 100'lük hesabı BEKLEMEDEN artırır (iyileşene bonus da verir).
    Örn: son 30 işlemde fomo:>1.2 başarısızsa → anında ek eksi puan."""
    m = _v11_mem()
    if not (V11_LEARN_ENABLED and V11_FAST_ENABLED):
        m["fast_adjust"] = {}
        return
    tr = m["trades"][-V11_FAST_WINDOW:]
    n_all = len(tr)
    if n_all < max(V11_FAST_MIN_N, 10):
        m["fast_adjust"] = {}
        return
    base_wr = sum(1 for t in tr if t.get("result") != "STOP") / n_all
    st: Dict[str, List[int]] = {}
    for t in tr:
        w = 1 if t.get("result") != "STOP" else 0
        for kk in (t.get("keys") or []):
            s = st.setdefault(kk, [0, 0])
            s[0] += 1; s[1] += w
    fast: Dict[str, float] = {}
    for kk, (n, wn) in st.items():
        if _v11_fam(kk) not in V11_ADJ_FAMILIES or n < V11_FAST_MIN_N:
            continue
        d = clamp(V11_FAST_GAIN * (wn / n - base_wr), -V11_FAST_PER_BUCKET, V11_FAST_PER_BUCKET)
        if abs(d) >= 0.5:
            fast[kk] = round(d, 1)
    m["fast_adjust"] = fast
    m["fast_meta"] = {"n": n_all, "base_wr": round(base_wr * 100, 1), "ts": time.time()}


# Dinamik ağırlık eşlemesi: skor bileşeni ↔ hangi kovalardan ölçülür
V11_W_NUMERIC = {"oi": ("oi",), "btc": ("btc1h", "btc4h"), "funding": ("fund",),
                 "rsi": ("rsi",), "structure": ("olay",)}
V11_W_FLAGS = {"order_block": "OB", "fvg": "FVG", "sweep": "SWEEP",
               "cvd": "CVD", "volume_profile": "VP", "orderbook": "KITAP"}


def v11_compute_weights(stats_k):
    """DİNAMİK FİLTRE AĞIRLIĞI: bileşenin kovaları arasındaki başarı YAYILIMI,
    o bileşenin ayırt ediciliğidir. Yayılım büyükse (örn OI kovaları %38↔%71)
    ağırlık artar; kovalar aynıysa (bilgi yok) ağırlık düşer. 0.6x–1.4x sınırlı."""
    if not V11_W_ENABLED:
        return {}
    def _mult(spread):
        return round(clamp(1.0 + V11_W_GAIN * (spread - V11_W_NEUTRAL), V11_W_MIN, V11_W_MAX), 3)
    w: Dict[str, float] = {}
    for comp, fams in V11_W_NUMERIC.items():
        wrs = [s["win"] / s["n"] for kk, s in stats_k.items()
               if "|" not in kk and _v11_fam(kk) in fams and s["n"] >= V11_MIN_BUCKET_N]
        if len(wrs) >= 2:
            w[comp] = _mult(max(wrs) - min(wrs))
    for comp, fl in V11_W_FLAGS.items():
        s1 = stats_k.get(f"onay:{fl}=1"); s0 = stats_k.get(f"onay:{fl}=0")
        if s1 and s0 and s1["n"] >= V11_MIN_BUCKET_N and s0["n"] >= V11_MIN_BUCKET_N:
            w[comp] = _mult(abs(s1["win"] / s1["n"] - s0["win"] / s0["n"]))
    return {kk: vv for kk, vv in w.items() if abs(vv - 1.0) >= 0.02}


def v11_recalc():
    """Son V11_WINDOW işlemden kova istatistiklerini ve puan ayarlarını yeniden hesaplar."""
    m = _v11_mem()
    win_tr = m["trades"][-V11_WINDOW:]
    n_all = len(win_tr)
    if n_all == 0:
        return None
    base_wr = sum(1 for t in win_tr if t.get("result") != "STOP") / n_all
    stats_k: Dict[str, Dict[str, float]] = {}
    reg_stat: Dict[str, List[int]] = {}
    for t in win_tr:
        w = 1 if t.get("result") != "STOP" else 0
        rg = (t.get("snap") or {}).get("regime")
        if rg:
            rs = reg_stat.setdefault(rg, [0, 0])
            rs[0] += 1; rs[1] += w
        for kk in (t.get("keys") or []):
            s = stats_k.setdefault(kk, {"n": 0, "win": 0, "R": 0.0})
            s["n"] += 1; s["win"] += w; s["R"] += safe_float(t.get("R"))
    adjust: Dict[str, float] = {}
    for kk, s in stats_k.items():
        if _v11_fam(kk) not in V11_ADJ_FAMILIES or s["n"] < V11_MIN_BUCKET_N:
            continue
        ref_wr = base_wr
        if "|" in kk:  # rejim-önekli kova → O REJİMİN kendi baz oranına göre ölç
            rg = kk.split("|", 1)[0]
            rn, rw = reg_stat.get(rg, [0, 0])
            if rn < V11_MIN_BUCKET_N:
                continue
            ref_wr = rw / rn
        wr = s["win"] / s["n"]
        d = clamp(V11_ADJ_GAIN * (wr - ref_wr), -V11_ADJ_PER_BUCKET, V11_ADJ_PER_BUCKET)
        if abs(d) >= 0.5:
            adjust[kk] = round(d, 1)
    m["adjust"] = adjust
    weights = v11_compute_weights(stats_k)
    m["weights"] = weights
    m["adjust_meta"] = {
        "base_wr": round(base_wr * 100, 1), "window_n": n_all,
        "n_total": int(m.get("n_closed", 0)), "ts": time.time(),
        "regime_wr": {rg: round(rw / rn * 100, 1) for rg, (rn, rw) in reg_stat.items() if rn},
        "weights": weights,
        "stats": {kk: {"n": int(s["n"]),
                       "wr": round(s["win"] / s["n"] * 100, 1),
                       "ev": round(s["R"] / s["n"], 3)}
                  for kk, s in stats_k.items()},
    }
    ordered = sorted(adjust.items(), key=lambda x: x[1], reverse=True)
    shown = ordered if len(ordered) <= 10 else (ordered[:5] + ordered[-5:])
    lines = [f"🧠 V11 ÖĞRENME GÜNCELLENDİ — son {n_all} işlem, genel başarı %{base_wr*100:.1f}"]
    if reg_stat:
        lines.append("Rejim: " + " | ".join(
            f"{rg} %{rw/rn*100:.0f} (n={rn})" for rg, (rn, rw) in sorted(reg_stat.items()) if rn))
    if weights:
        lines.append("Ağırlık: " + " ".join(
            f"{kk}×{vv:g}" for kk, vv in sorted(weights.items(), key=lambda x: abs(x[1]-1.0), reverse=True)[:6]))
    for kk, d in shown:
        s = stats_k[kk]
        lines.append(f"{kk} → başarı %{s['win']/s['n']*100:.0f} (n={int(s['n'])}) → {d:+.1f}p")
    if not adjust:
        lines.append(f"Yeterli örneği (n≥{V11_MIN_BUCKET_N}) olan kova yok — ayar uygulanmadı.")
    logger.info("V11 RECALC: %d işlem, %d kova ayarlandı", n_all, len(adjust))
    return "\n".join(lines)


def v11_maybe_recalc():
    """Her V11_RECALC_EVERY kapanışta bir yeniden hesap tetikler."""
    if not V11_LEARN_ENABLED:
        return None
    m = _v11_mem()
    n = int(m.get("n_closed", 0))
    if n >= V11_RECALC_EVERY and (n - int(m.get("last_recalc_n", 0))) >= V11_RECALC_EVERY:
        m["last_recalc_n"] = n
        try:
            return v11_recalc()
        except Exception as e:
            logger.exception("v11_recalc hata: %s", e)
    return None


async def cmd_v11(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """V11 öğrenme raporu: kova başarı oranları, EV ve aktif puan ayarları."""
    m = _v11_mem()
    meta = m.get("adjust_meta", {}) or {}
    st = meta.get("stats", {}) or {}
    n = int(m.get("n_closed", 0))
    nxt = V11_RECALC_EVERY - (n - int(m.get("last_recalc_n", 0))) if n >= V11_RECALC_EVERY else V11_RECALC_EVERY - n
    lines = [
        "🧠 V11 ÖĞRENEN SKOR",
        f"Durum: {'AÇIK' if V11_LEARN_ENABLED else 'KAPALI'} | Kapanan işlem: {n} | Sonraki hesap: {max(0, nxt)} işlem sonra",
        f"Aktif ayar: {len(m.get('adjust', {}))} kova | Pencere: son {V11_WINDOW} | Min örnek: {V11_MIN_BUCKET_N} | Kova ±{V11_ADJ_PER_BUCKET:g}p, toplam ±{V11_ADJ_TOTAL:g}p",
    ]
    if meta:
        lines.append(f"Genel başarı: %{meta.get('base_wr', '-')} ({meta.get('window_n', '-')} işlem penceresi)")
    rw = meta.get("regime_wr") or {}
    if rw:
        lines.append("Rejim başarı: " + " | ".join(f"{kk} %{vv}" for kk, vv in sorted(rw.items())))
    wgt = m.get("weights") or {}
    if wgt:
        lines.append("Dinamik ağırlık: " + " ".join(
            f"{kk}×{vv:g}" for kk, vv in sorted(wgt.items(), key=lambda x: abs(x[1]-1.0), reverse=True)[:8]))
    fm = m.get("fast_meta") or {}
    fa = m.get("fast_adjust") or {}
    if fm:
        lines.append(f"⚡ Hızlı pencere (son {fm.get('n','-')} işlem, genel %{fm.get('base_wr','-')}): {len(fa)} aktif ayar")
        for kk, d in sorted(fa.items(), key=lambda x: abs(x[1]), reverse=True)[:6]:
            lines.append(f"  ⚡{kk} → {d:+.1f}p")
    adj = m.get("adjust", {})
    if adj:
        lines.append("— Aktif puan ayarları —")
        for kk, d in sorted(adj.items(), key=lambda x: abs(x[1]), reverse=True)[:14]:
            s = st.get(kk, {})
            lines.append(f"{kk}: n={s.get('n','?')} başarı %{s.get('wr','?')} EV {s.get('ev','?')}R → {d:+.1f}p")
    elif st:
        lines.append("— İstatistik (ayar için örnek henüz yetersiz) —")
        for kk, s in sorted(st.items(), key=lambda x: x[1]["n"], reverse=True)[:12]:
            lines.append(f"{kk}: n={s['n']} başarı %{s['wr']} EV {s['ev']}R")
    else:
        bks = m.get("buckets", {})
        if bks:
            lines.append(f"— İlk sayaçlar (ilk hesap {V11_RECALC_EVERY}. işlemde) —")
            for kk, b in sorted(bks.items(), key=lambda x: x[1]["n"], reverse=True)[:12]:
                if b["n"]:
                    lines.append(f"{kk}: n={b['n']} başarı %{b['win']/b['n']*100:.0f} EV {b['R']/b['n']:+.2f}R")
        else:
            lines.append("Henüz kapanan işlem yok — kayıt, sinyaller kapanmaya başlayınca dolar.")
    await update.message.reply_text("\n".join(lines))
...
async def cmd_v10(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    lines = [
        ...
        f"V11 öğrenme: {'AÇIK' if V11_LEARN_ENABLED else 'KAPALI'} | Kayıt: {int(_v11_mem().get('n_closed', 0))} | Aktif ayar: {len(_v11_mem().get('adjust', {}))} kova → /v11",
    ]

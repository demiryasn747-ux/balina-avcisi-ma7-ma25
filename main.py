...
VERSION_NAME = "Balina Avcısı V5.2.7 HİBRİT ONAYLI + MA7/MA25 1H 200 COIN BİRLEŞİK"
...
LIQUIDATION_BUFFER = float(os.getenv("LIQUIDATION_BUFFER", "0.15"))  # Stop ile likidasyon arası min mesafe %
DEFAULT_MARGIN_USDT = float(os.getenv("DEFAULT_MARGIN_USDT", "100"))  # Örnek marj miktarı

# Kaldıraçlı stop hesapla: yüksek kaldıraçta daralır, düşükte geniş kalır
def calc_leveraged_stop_pct(base_pct: float) -> float:
    if LEVERAGE <= 1:
        return base_pct
    # Kaldıraç arttıkça stop daralır ama min sınırı geçmez
    adjusted = min(base_pct, 0.08 / LEVERAGE)
    return max(MIN_STOP_PCT, adjusted)
...
async def analyze_symbol(symbol: str, tickers24: Dict[str, Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    # Likidasyon kontrolü (SHORT)
    liq_safe_v5, liq_gap_v5 = check_stop_vs_liquidation(entry, stop, "SHORT")
    if not liq_safe_v5 and LEVERAGE > 5:
        logger.warning("V5 sinyal RED (likidasyon riski): %s | kaldıraç=%sx", symbol, LEVERAGE)
        return None
...
def _build_ma_result(symbol: str, direction: str, k1h: List[List[Any]], ma7: List[float], ma25: List[float]) -> Optional[Dict[str, Any]]:
    ...
    # Likidasyon güvenlik kontrolü - stop likidasyondan önce olmalı
    liq_safe, liq_gap = check_stop_vs_liquidation(entry, targets["stop"], direction)
    if not liq_safe and LEVERAGE > 5:

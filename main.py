...
VERSION_NAME = "Balina Avcısı V8.1 ULTIMATE (OI + Funding + S/R İlişki Filtresi)"
...
# V8.1 S/R İLİŞKİ FİLTRESİ:
#   SHORT: direnç destekten uzaksa (range dibinde short) → blokla
#   LONG:  destek dirençten uzaksa (range tepesinde long) → blokla
# Veri bulgusu: OL/ALGO short'ları direnç>destek ile geldi ve stop oldu.
SR_RELATION_FILTER_ENABLED = os.getenv("SR_RELATION_FILTER_ENABLED", "true").lower() == "true"
...
def tr_month_key(ts: Optional[float] = None) -> str:
    dt = datetime.fromtimestamp(ts, TZ) if ts else tr_now()
    return dt.strftime("%Y-%m")


# =========================================================
# AYLIK PERFORMANS HAFIZASI
# Her sinyali ve sonucunu ay bazında biriktirir.
# =========================================================
def _empty_month() -> Dict[str, Any]:
    return {
        "signals": 0, "long": 0, "short": 0,
        "tp1": 0, "tp2": 0, "tp3": 0, "stop": 0,
        "long_tp": 0, "long_stop": 0,
        "short_tp": 0, "short_stop": 0,
        "pnl_pct": 0.0,
        "open": 0,  # gönderilmiş ama henüz sonuçlanmamış
    }

def record_monthly_signal(direction: str, ts: Optional[float] = None) -> None:
    """Sinyal gönderildiğinde çağrılır."""
    mk = tr_month_key(ts)
    months = memory.setdefault("monthly_stats", {})
    m = months.setdefault(mk, _empty_month())
    m["signals"] = int(m.get("signals", 0)) + 1
    m["open"] = int(m.get("open", 0)) + 1
    if direction == "LONG":
        m["long"] = int(m.get("long", 0)) + 1
    elif direction == "SHORT":
        m["short"] = int(m.get("short", 0)) + 1

def record_monthly_result(direction: str, result: str, pnl_pct: float,
                          signal_ts: Optional[float] = None) -> None:
    """Sinyal TP/STOP ile sonuçlandığında çağrılır. Ay, sinyalin atıldığı aya yazılır."""
    mk = tr_month_key(signal_ts)
    months = memory.setdefault("monthly_stats", {})
    m = months.setdefault(mk, _empty_month())
    m["open"] = max(0, int(m.get("open", 0)) - 1)
    m["pnl_pct"] = round(float(m.get("pnl_pct", 0.0)) + pnl_pct, 4)

    res = str(result).upper()
    is_tp = res.startswith("TP")
    if res == "TP1":
        m["tp1"] = int(m.get("tp1", 0)) + 1
    elif res == "TP2":
        m["tp2"] = int(m.get("tp2", 0)) + 1
    elif res == "TP3":
        m["tp3"] = int(m.get("tp3", 0)) + 1
    elif res == "STOP":
        m["stop"] = int(m.get("stop", 0)) + 1

    if direction == "LONG":
        if is_tp:
            m["long_tp"] = int(m.get("long_tp", 0)) + 1
        elif res == "STOP":
            m["long_stop"] = int(m.get("long_stop", 0)) + 1
    elif direction == "SHORT":
        if is_tp:
            m["short_tp"] = int(m.get("short_tp", 0)) + 1
        elif res == "STOP":
            m["short_stop"] = int(m.get("short_stop", 0)) + 1

def build_monthly_report(month_key: Optional[str] = None) -> str:
    """Aylık performans raporu metni."""
    mk = month_key or tr_month_key()
    months = memory.get("monthly_stats", {})
    m = months.get(mk)
    if not m:
        return f"📅 {mk} için henüz veri yok."

    signals = int(m.get("signals", 0))
    long_c = int(m.get("long", 0))
    short_c = int(m.get("short", 0))
    tp1 = int(m.get("tp1", 0)); tp2 = int(m.get("tp2", 0)); tp3 = int(m.get("tp3", 0))
    stop = int(m.get("stop", 0)); open_c = int(m.get("open", 0))
    total_tp = tp1 + tp2 + tp3
    closed = total_tp + stop
    win_rate = (total_tp * 100.0 / closed) if closed > 0 else 0.0
    pnl = float(m.get("pnl_pct", 0.0))

    long_tp = int(m.get("long_tp", 0)); long_stop = int(m.get("long_stop", 0))
    short_tp = int(m.get("short_tp", 0)); short_stop = int(m.get("short_stop", 0))
    long_closed = long_tp + long_stop
    short_closed = short_tp + short_stop
    long_wr = (long_tp * 100.0 / long_closed) if long_closed > 0 else 0.0
    short_wr = (short_tp * 100.0 / short_closed) if short_closed > 0 else 0.0

    return (
        f"📅 AYLIK PERFORMANS — {mk}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 Toplam sinyal: {signals}\n"
        f"   ├─ LONG: {long_c}\n"
        f"   └─ SHORT: {short_c}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🎯 SONUÇLAR (kapanmış: {closed})\n"
        f"   ├─ ✅ TP1: {tp1}\n"
        f"   ├─ ✅ TP2: {tp2}\n"
        f"   ├─ ✅ TP3: {tp3}\n"
        f"   ├─ ❌ STOP: {stop}\n"
        f"   └─ ⏳ Açık: {open_c}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"📈 Win Rate: %{win_rate:.1f} ({total_tp}/{closed})\n"
        f"💰 Net P&L: %{pnl:+.2f}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🔻 SHORT: {short_tp}W/{short_stop}L | %{short_wr:.1f}\n"
        f"🔺 LONG: {long_tp}W/{long_stop}L | %{long_wr:.1f}"
    )
...
def ensure_memory_shape() -> None:
    ...
    memory.setdefault("monthly_stats", {})
...
def _build_ma_result(symbol: str, direction: str, k1h: List[List[Any]], ma7: List[float], ma25: List[float]) -> Optional[Dict[str, Any]]:
    ...
    if direction == "SHORT":
        ...
        if safe_float(sr.get("support_diff_pct", 0)) < SHORT_MIN_SUPPORT_DIFF_PCT:
            return None
        # V8.1 İLİŞKİ FİLTRESİ: SHORT'ta direnç destekten uzaksa = range dibinde short = sıçrama riski
        if SR_RELATION_FILTER_ENABLED:
            res_diff = safe_float(sr.get("resistance_diff_pct", 0))
            sup_diff = safe_float(sr.get("support_diff_pct", 0))
            if res_diff >= sup_diff:
                logger.info("MA SHORT RED (S/R ilişki): %s direnç %%%.2f >= destek %%%.2f (range dibinde short)",
                            symbol, res_diff, sup_diff)
                return None
    ...
    if direction == "LONG":
        ...
        if safe_float(sr.get("resistance_diff_pct", 0)) < LONG_MIN_RESISTANCE_DIFF_PCT:
            return None
        # V8.1 İLİŞKİ FİLTRESİ (mirror): LONG'ta destek dirençten uzaksa = range tepesinde long = düşüş riski
        if SR_RELATION_FILTER_ENABLED:
            res_diff = safe_float(sr.get("resistance_diff_pct", 0))
            sup_diff = safe_float(sr.get("support_diff_pct", 0))
            if sup_diff >= res_diff:
                logger.info("MA LONG RED (S/R ilişki): %s destek %%%.2f >= direnç %%%.2f (range tepesinde long)",
                            symbol, sup_diff, res_diff)
                return None
...
def mark_ma_sent(res: Dict[str, Any]) -> None:
    ...
    # AYLIK HAFIZA: sinyali kaydet
    record_monthly_signal(res["direction"], sent_ts)
...
async def check_ma_followups() -> None:
    ...
    for key, rec in list(follows.items()):
        ...
        if ok:
            ...
            result_str = str(hit.get("result"))
            if result_str == "STOP":
                memory["stats"]["ma_stop"] = int(memory["stats"].get("ma_stop", 0)) + 1
            elif result_str.startswith("TP"):
                memory["stats"]["ma_tp"] = int(memory["stats"].get("ma_tp", 0)) + 1
            # AYLIK HAFIZA: sonucu + gerçek P&L kaydet
            direction = str(rec.get("direction", ""))
            entry = safe_float(rec.get("entry", 0))
            result_price = safe_float(hit.get("price", 0))
            sent_ts = safe_float(rec.get("sent_ts", 0)) or None
            pnl_pct = 0.0
            if entry > 0 and result_price > 0:
                if direction == "SHORT":
                    pnl_pct = (entry - result_price) / entry * 100.0
                elif direction == "LONG":
                    pnl_pct = (result_price - entry) / entry * 100.0
            # AYNI_1M_MUMDA_STOP_TP belirsiz → STOP say (kötü senaryo, muhafazakar)
            norm_result = result_str if result_str in ("TP1", "TP2", "TP3", "STOP") else "STOP"
            if norm_result == "STOP" and pnl_pct > 0:
                pnl_pct = -abs(pnl_pct)  # belirsiz durumda muhafazakar
            record_monthly_result(direction, norm_result, pnl_pct, sent_ts)
...
async def cmd_aylik(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Aylık performans raporu. /aylik veya /aylik 2026-05"""
    month_key = None
    if context.args:
        arg = context.args[0].strip()
        # "2026-05" formatı veya "05" / "5" → bu yılın o ayı
        if len(arg) == 7 and "-" in arg:
            month_key = arg
        elif arg.isdigit():
            month_key = f"{tr_now().year}-{int(arg):02d}"
    msg = build_monthly_report(month_key)
    # Tüm ayların listesi varsa altına ekle
    months = memory.get("monthly_stats", {})
    if len(months) > 1:
        other = sorted([k for k in months.keys() if k != (month_key or tr_month_key())], reverse=True)
        if other:
            msg += "\n━━━━━━━━━━━━━━━━━━━━\n📆 Diğer aylar: " + ", ".join(other[:6])
            msg += "\n(görmek için: /aylik 2026-04)"
...
def build_app():
    ...
    application.add_handler(CommandHandler("performans", cmd_aylik))
    return application

...
async def _prepare_ma_data(symbol: str) -> Optional[Tuple[str, List[List[Any]], List[float], List[float]]]:
    ...
    c = closes(k1h)
    # MA periyotları ENV'den ayarlanabilir. Varsayılan: MA5 / MA10 (grafikteki MA5, MA10).
...
def build_ma_signal_message(res: Dict[str, Any]) -> str:
    ...
    return (
        f"🚨 {VERSION_NAME} - MA{MA_FAST_PERIOD}/MA{MA_SLOW_PERIOD} {res['timeframe']} - {res['direction']} AL\n"
        f"Saat: {tr_str()}\n"
        f"Coin: {res['symbol']}\n"
        f"Motor: {res['direction']} MOTORU\n"
        f"Zaman dilimi: {res.get('timeframe', MA_KLINE_INTERVAL)}\n"
        f"Kural: MA{MA_FAST_PERIOD} / MA{MA_SLOW_PERIOD} KAPANMIŞ mum kesişimi (repainting önlendi)\n"
        f"{res['entry_note']}\n"
        f"MA{MA_FAST_PERIOD}: {fmt_num(res['ma7'])}\n"
        f"MA{MA_SLOW_PERIOD}: {fmt_num(res['ma25'])}\n"
...
def build_ma_followup_message(rec: Dict[str, Any], hit: Dict[str, Any]) -> str:
    ...
    return (
        f"⏱ MA{MA_FAST_PERIOD}/MA{MA_SLOW_PERIOD} TP/STOP TAKİP\n"
        f"{title}\n"
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        "/hot - sıcak coinler\n"
        "/ma BTCUSDT - MA tek coin\n"
        "/ma_status - MA motor durumu\n"
...
async def cmd_ma(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if res:
        ...
        await update.message.reply_text(f"{symbol} için su an MA temas-kesisim yok.")
...
async def cmd_ma_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        f"💓 MA{MA_FAST_PERIOD}/MA{MA_SLOW_PERIOD} 1H 200 COIN DURUM\n"
        f"Saat: {tr_str()}\n"
        ...
        f"Kural: MA{MA_FAST_PERIOD} altına MA{MA_SLOW_PERIOD} = SHORT | MA{MA_FAST_PERIOD} üstüne MA{MA_SLOW_PERIOD} = LONG\n"
